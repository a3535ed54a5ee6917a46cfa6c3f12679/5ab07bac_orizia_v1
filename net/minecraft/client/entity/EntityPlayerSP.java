...
public class EntityPlayerSP extends AbstractClientPlayer
{
     ...
     * sprinting.
     */
    public boolean tsprint = false;
    public boolean tsneak = false;
    
    public static boolean tsprintenabled;
    public static boolean tsneakenabled;
   
    public byte READ_KEY_PUSH_Sneak=0;
    public byte READ_KEY_PUSH_Sprint=0;
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            READ_KEY_PUSH_Sneak  = (byte)(((READ_KEY_PUSH_Sneak  << 1) | (byte)(this.mc.gameSettings.ToggleSneak.getIsKeyPressed()?1:0))&3);//GET VALEUR sneak ;
            READ_KEY_PUSH_Sprint = (byte)(((READ_KEY_PUSH_Sprint << 1) | (byte)(this.mc.gameSettings.ToggleSprint.getIsKeyPressed()?1:0))&3);//GET VALEUR sprint;
            if (READ_KEY_PUSH_Sprint == 1){
                if(tsprint){tsneak = false;}
                tsprint =! tsprint;
            }
            if (READ_KEY_PUSH_Sneak == 1){
                if(tsneak){tsprint = false;}
                tsneak =! tsneak;
            }
            ...
            if(!tsprint) tsprintenabled = false;
            if(tsneak)
            {
            	
            	tsneakenabled = true;
            	if(tsneak && tsprint){
            		tsprintenabled= false;
            	} 
            	this.movementInput.sneak = true;
            	this.ySize = 0.2F;
